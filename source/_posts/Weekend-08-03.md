...
理解：数轴上从 $0$ 开始每隔 $b - 1$ 个数划为一个小区间，形成若干段长度为 $b$ 的小区间，从左到右依次编号为 $0,1,2,\dots$。这里每个数的编号就代表着 $\left\lfloor\frac{a}{b}\right\rfloor$ 的值。
...
首先，由上述定理，原文题等价于求 $\left\lfloor\frac{N \pmod{M \times 10007}}{M}\right\rfloor$，对于每一个循环的部分考虑二进制拆分，分别预处理每一位的贡献，再合并就好了。
